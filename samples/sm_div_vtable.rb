...
PDFGen::document PDF::Writer.new, 1.cm do
  ...
  div :pad_left => 2.cm, :pad_right => 1.5.cm do
    table :data_source => table_data,  :repeat_header_on_each_page => true,
      ...
      header do
        row do
          cell ds.columns[0], :text_color => Color::RGB::DarkGrey, :font_size => 10
          cell ds.columns[1], :text_color => Color::RGB::DarkGrey, :font_size => 10
          cell ds.columns[2], :text_color => Color::RGB::DarkGrey, :font_size => 10
          cell ds.columns[3], :text_color => Color::RGB::DarkGrey, :font_size => 10
      end
      end
      ...
      body do
        ds.each do |datarow|
          row do
            ...
            cell :div, :width => av_width / 4, :paddings => 0.1.cm do
                  image(open(File.expand_path("#{datarow[3]}"), "rb") { |file| file.read }, :width => av_width)
                  caption "Sequence", :text_color => Color::RGB::Grey, :justification => :center
                  caption "Cameraman - Pal Smith", :justification => :center
                  end
          end
        end
      end
